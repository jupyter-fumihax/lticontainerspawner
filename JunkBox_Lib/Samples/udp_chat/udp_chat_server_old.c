...
int main(int argc, char** argv)
{
    ...
    Loop{
        ...
        cc = udp_recv(sock, rmsg, RECVBUFSZ-1, &cl_addr);
        ...
        if (cc>0) {
            ...
            if (command=='S') {
                if (operand!=NULL){
                    if (host==NULL) {
                        ...
                        kcode = CODE_EUC;
                        if (knji!=NULL) {
                            if (!strncasecmp(knji, "CODE_SJIS", 4)) {
            ...
            /*
                ...
                if (operand!=NULL) {
                    if (!strncasecmp(operand, "CODE_SJIS", 4)) {
                        host->kcode = CODE_SJIS;
                    }
                    else if (!strncasecmp(operand, "CODE_EUC", 3)) {
                        host->kcode = CODE_EUC;
                    }
                }
            }*/
...
void  udp_send_message(int sock, char* mesg, struct host_list* nx)
{
    ...
    udp_send(sock, sendmesg, 0, &sl_addr);
    free(sendmesg);
}
